...
async function addTransaction(req, res) {
   ...
   try {
      ...
      const tx1 = new Transaction(fromAddress, toAddress || 'some public key', amount || 10)
...
async function getWallet(req, res) {
   try {
      ...
      const walletAddress = await blockchainService.getWalletAddress(privateKey)
      const balance = popCoin.getBalanceOfAddress(walletAddress) + ''
      res.send(balance)
   } catch (error) {
      logger.error('Invalid key', error)
